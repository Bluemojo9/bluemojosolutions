...
class Merlin {
	...
	*/
	function required_classes() {
		...
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-merlin-cptui-importer.php';
		...
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-merlin-rev-importer.php';
	...
	*/
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'crsine' ),
		);
	...
	*/
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'crsine' );
		}
		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'crsine' );
		}
		// Set defaults.
		$defaults = array(
			'icon' => '',
	...
	*/
	public function steps() {
		$this->steps = array(
			'welcome' => array(
				'name' => esc_html__( 'Welcome', 'crsine' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'crsine' ),
				'view' => array( $this, 'content' ),
			);
		}
		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'crsine' ),
	...
	*/
	...
	protected function server_requirements($step) {
		...
		$upload_max_filesize = intval(substr(size_format( wp_max_upload_size() ), 0, -1));
		if ($upload_max_filesize < 6 ) {
		...
		<table class="widefat" cellspacing="0">
			<tbody>
				<?php if ( function_exists( 'ini_get' ) ) : ?>
					...
					if ( function_exists( 'phpversion' ) ) {
						...
						<?php if ($php_check == 'error'): ?>
							<tr>
								<td><?php esc_html_e( 'PHP Version', 'wavo' ); ?></td>
								<td>
									<span class="dashicons dashicons-warning red"></span> <span><?php echo esc_html($phpversion); ?></span>
									<div><small><?php esc_html_e('Recommended: ', 'wavo'); ?><strong>7.0 or higher</strong></small></div>
								</td>
							</tr>
						<?php endif; ?>
					<?php	} ?>
					<?php if ($max_input_vars == 'error'):
						...
						?>
						<tr>
							<td><?php esc_html_e( 'max_input_vars', 'wavo' ); ?></td>
							<td>
								<span class="dashicons dashicons-warning <?php echo esc_attr($color); ?>"></span> <span><?php echo ini_get('max_input_vars'); ?></span>
								<div><small><?php esc_html_e('Recommended: ', 'wavo'); ?><strong>3000</strong></small></div>
							</td>
						</tr>
					<?php endif; ?>
					<?php if ($memory_limit == 'error'): ?>
						<tr>
							<td><?php esc_html_e( 'memory_limit', 'wavo' ); ?></td>
							<td>
								<span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('memory_limit'); ?></span>
								<div><small><?php esc_html_e('Recommended: ', 'wavo'); ?><strong>256M</strong></small></div>
							</td>
						</tr>
					<?php endif; ?>
					<?php if ($max_execution_time == 'error'): ?>
						<tr>
							<td><?php esc_html_e( 'max_execution_time', 'wavo' ); ?></td>
							<td>
								<span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('max_execution_time'); ?></span>
								<div><small><?php esc_html_e('Recommended: ', 'wavo'); ?><strong>300</strong></small></div>
							</td>
						</tr>
					<?php endif; ?>
					<?php if ($upload_max_filesize == 'error'): ?>
						<tr>
							<td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'wavo' ); ?>:</td>
							<td>
								<span class="dashicons dashicons-warning red"></span> <span><?php echo size_format( wp_max_upload_size() ); ?></span>
								<div><small><?php esc_html_e('Recommended: ' , 'wavo'); ?><strong>12M</strong></small></div>
							</td>
						</tr>
					<?php endif; ?>
				<?php endif; ?>
				...
				$posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'wavo' ) . '">[?]</a>';
				if (  (function_exists( 'fsockopen' ) || function_exists( 'curl_init' ) ) ) {
		...
		<?php if ( $go == false) { ?>
			<?php if ( $step == 'welcome' ) : ?>
				<p class="server_check_notice step_welcome red"><?php echo sprintf( __( 'Your server does not match the recommended settings. You might not be able to import the full demos.', 'wavo' ), '<span class="underline">', '</span>' ); ?></p>
				<p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s, or proceed with caution :(', 'wavo' ), '<a href="https://ninetheme.com/docs/wordpress-server-requirements/" target="_blank">', '</a>' ); ?></p>
			<?php else: ?>
				<p class="server_check_notice step_import red"><?php echo sprintf( __( 'Your server does not match the recommended settings. %sImport the BASIC Demo%s only, or adjust your settings.', 'wavo' ), '<span class="underline">', '</span>' ); ?></p>
			<?php endif; ?>
		<?php } ?>
		<?php
	}
	...
	*/
	protected function license() {
		$purchase_code = get_option('envato_purchase_code_45172760');
		if ( $purchase_code ) {
		...
		$redirected_licence = isset( $_POST['ninetheme_connect_licence'] ) || 'yes' == get_option( 'ninetheme_deactivated_licence' ) ? ' redirected-licence' : '';
		?>
	...
	*/
	protected function plugins() {
		$purchase_code = get_option('envato_purchase_code_45172760');
		if(!$purchase_code) {
			echo '<p>' . esc_html__('Please verify the purchase code first.', 'wavo') . '</p>';
			    echo sprintf( '<a class="relative" href="%s">%s</a>',
			    esc_url( admin_url( '/' ) ),
			    esc_html__('Back Admin Page', 'wavo')
			    );
			exit(0);
		}
		// Variables.
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
										<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'crsine' ); ?>">
											<?php esc_html_e( 'req', 'crsine' ); ?>
										</span>
									</span>
								</label>
							</li>
	...
	*/
	protected function content() {
		...
		$landing_url = function_exists( 'r_energy_merlin_local_import_files' ) ? r_energy_merlin_local_import_files() : array();
				...
				<div class="merlin__select-control-wrapper">
					<select class="merlin__select-control js-merlin-demo-import-select">
						<?php foreach ( $this->import_files as $index => $import_file ) : ?>
							<option data-url="<?php echo esc_attr( $import_file['import_preview_url'] ); ?>" value="<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $import_file['import_file_name'] ); ?></option>
						<?php endforeach; ?>
					</select>
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'crsine' ); ?>">
							<?php echo wp_kses( $this->svg( array( 'icon' => 'downarrow' ) ), $this->svg_allowed_html() ); ?>
						</span>
					</div>
					<div class="merlin__select-control-preview">
						<?php if ( $landing_url ) : ?>
							<a class="all-demos" href="<?php echo esc_url( $landing_url ); ?>" target="_blank"><?php esc_html_e( 'See All Demos', 'wp-merlin' ); ?></a>
						<?php endif; ?>
						<?php if ( !empty( $import_file['import_preview_url'] ) ) : ?>
							<a class="current-demo" href="<?php echo esc_attr( $import_file['import_preview_url'] ); ?>" target="_blank"><?php esc_html_e( 'See ', 'wp-merlin' ); ?><span><?php echo esc_html( $this->import_files[0]['import_file_name'] ); ?></span></a>
						<?php endif; ?>
					</div>
				</div>
			<?php endif; ?>
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'crsine' ),
				)
			);
		}
		if ( empty( $license_key ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'crsine' ),
				)
			);
		}
		...
		switch ($output->status) {
			case 200:
				add_option('envato_purchase_code_45172760', $license_key, '', 'yes');
				if ( 'yes' == get_option('ninetheme_deactivated_licence') ) {
					delete_option( 'ninetheme_deactivated_licence' );
					$href = esc_url( admin_url('admin.php?page=ninetheme_about_page') );
					$btn = esc_html__( 'Return to the Dashboard', 'crsine' );
					$title = esc_html__( 'Activated', 'crsine' );
					$wait = esc_html__( 'The activation is done!', 'crsine' );
					wp_send_json(array('success' => true,'complete' => true, 'href' => $href,'wait' => $wait,'title' => $title,'message' => esc_html__($output->msg, 'crsine' )));
				} else {
					wp_send_json(array('success' => true, 'message' => esc_html__($output->msg, 'crsine' )));
				}
			break;
			case 400:
				wp_send_json(array('success' => false, 'message' => esc_html__($output->msg, 'crsine' )));
			break;
			case 404:
				wp_send_json(array('success' => false, 'message' => esc_html__($output->msg, 'crsine' )));
			break;
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'crsine' );
			}
		} else {
			...
			if ( false === $license_data->success ) {
				switch ( $license_data->error ) {
					...
					$message = sprintf(
						...
						esc_html__( 'Your license key expired on %s.', 'crsine' ),
						date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
					);
					break;
					case 'revoked':
					$message = esc_html__( 'Your license key has been disabled.', 'crsine' );
					break;
					case 'missing':
					$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'crsine' );
					break;
					case 'invalid':
					case 'site_inactive':
					$message = esc_html__( 'Your license is not active for this URL.', 'crsine' );
					break;
					case 'item_name_mismatch':
					/* translators: EDD Item Name */
					$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'crsine' ), $this->edd_item_name );
					break;
					case 'no_activations_left':
					$message = esc_html__( 'Your license key has reached its activation limit.', 'crsine' );
					break;
					default:
					$message = esc_html__( 'An error occurred, please try again.', 'crsine' );
					break;
...
";
	...
	*/
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2' => - 1,
					'message' => esc_html__( 'Activating', 'crsine' ),
				);
				break;
			}
		}
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2' => - 1,
					'message' => esc_html__( 'Updating', 'crsine' ),
				);
				break;
			}
		}
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2' => - 1,
					'message' => esc_html__( 'Installing', 'crsine' ),
				);
				break;
			}
		}
		if ( $json ) {
			...
			$json['message'] = esc_html__( 'Installing', 'crsine' );
			wp_send_json( $json );
		} else {
			wp_send_json(
				array(
					'done' => 1,
					'message' => esc_html__( 'Success', 'crsine' ),
				)
			);
		}
		exit;
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			wp_send_json_error(
				array(
					'error' => 1,
					'message' => esc_html__( 'Invalid content!', 'crsine' ),
				)
			);
		}
		$json         = false;
		...
		if ( $json ) {
			...
			wp_send_json(
				array(
					'error' => 1,
					'message' => esc_html__( 'Error', 'crsine' ),
					'logs' => '',
					'errors' => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			wp_send_json_error(
				array(
					'error' => 1,
					'message' => esc_html__( 'Invalid data!', 'crsine' ),
				)
			);
		}
		$selected_import = intval( $_POST['selected_index'] );
	...
	*/
	public function get_import_data_info( $selected_import_index = 0 ) {
		...
		if ( !empty( $this->import_files[ $selected_import_index ]['local_import_codestar_file'] ) ) {
			$import_data['codestar'] = true;
		}
		if (
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title' => esc_html__( 'Content', 'crsine' ),
				'description' => esc_html__( 'Demo content data.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title' => esc_html__( 'Widgets', 'crsine' ),
				'description' => esc_html__( 'Sample widgets data.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title' => esc_html__( 'Revolution Slider', 'crsine' ),
				'description' => esc_html__( 'Sample Revolution sliders data.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title' => esc_html__( 'Options', 'crsine' ),
				'description' => esc_html__( 'Sample theme options data.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title' => esc_html__( 'Redux Options', 'crsine' ),
				'description' => esc_html__( 'Redux framework options.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( ! empty( $import_files['option_tree'] ) ) {
			$content['option_tree'] = array(
				'title' => esc_html__( 'Option Tree Options', 'crsine' ),
				'description' => esc_html__( 'Option Tree framework options.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( ! empty( $import_files['codestar'] ) ) {
			$content['codestar'] = array(
				'title' => esc_html__( 'Codestar Options', 'crsine' ),
				'description' => esc_html__( 'Codestar framework options.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
				'install_callback' => array( 'Merlin_Codestar_Importer', 'import' ),
				'checked' => $this->is_possible_upgrade() ? 0 : 1,
				'data' => $import_files['codestar'],
			);
		}
		if ( ! empty( $import_files['cptui'] ) ) {
			$content['cptui'] = array(
				'title' => esc_html__( 'CPTUI Options', 'crsine' ),
				'description' => esc_html__( 'CPTUI framework options.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title' => esc_html__( 'After import setup', 'crsine' ),
				'description' => esc_html__( 'After import setup.', 'crsine' ),
				'pending' => esc_html__( 'Pending', 'crsine' ),
				'installing' => esc_html__( 'Installing', 'crsine' ),
				'success' => esc_html__( 'Success', 'crsine' ),
	...
	*/
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'crsine' );
		$data['proceed']  = 'true';
	...
	*/
	public function get_import_files_paths( $selected_import_index ) {
		...
		if (!empty($selected_import_data['local_import_codestar_file'])) {
			$codestar_items = array();
			foreach ( $selected_import_data['local_import_codestar_file'] as $codestar_item ) {
				if ( file_exists( $codestar_item['file_path'] ) ) {
					$codestar_items[] = array(
						'option_name'	=> $codestar_item['option_name'],
						'file_path'	=> $codestar_item['file_path']
					);
					$codestar_items[] = $codestar_item;
				}
			}
			$import_files['codestar'] = $codestar_items;
		}
		// Get redux import file as well. If defined!
